...
var module_default2 = usePassiveLayoutEffect;

// node_modules/@react-hook/event/dist/module/index.js
function useEvent(target, type, listener, cleanup) {
  const storedListener = React4.useRef(listener);
  const storedCleanup = React4.useRef(cleanup);
  module_default2(() => {
...
var useWindowSize = (options = emptyObj) => {
  ...
  const [size, setDebouncedSize] = useDebounce(typeof document === "undefined" ? [initialWidth, initialHeight] : getSize, wait, leading);
  const setSize = () => setDebouncedSize(getSize);
  module_default3(win, "resize", setSize);
  module_default3(win, "orientationchange", setSize);
  return size;
};
